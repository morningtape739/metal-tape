...
(function () {
  ...
  const BTC_ADDRESS = 'bc1qej3uqlv202lxyz7zph8du9mh25rzyqtxs8zsvj';
  ...
  function refineBandName(title, currentBand, bandIndex) {
    ...
    if (title.match(/^[^:]{2,40}:/) || title.match(/^[^—–]{2,40}[—–]/)) return currentBand;
  ...
  const MetalTape = {
    ...
    toggleBtc(e) {
      if (e) { e.stopPropagation(); e.preventDefault(); }
      state.btcOpen = !state.btcOpen; state.btcCopied = false;
      if (state.btcOpen) { state.watchlistOpen = false; state.searchOpen = false; }
      render();
    },
    copyBtc(e) {
      if (e) { e.stopPropagation(); e.preventDefault(); }
      navigator.clipboard.writeText(BTC_ADDRESS).then(() => {
        state.btcCopied = true; render();
        setTimeout(() => { state.btcCopied = false; render(); }, 2500);
      });
    },
  ...
  // Close BTC panel on outside click
  document.addEventListener('click', function(e) {
    if (state.btcOpen && !e.target.closest('.btc-panel') && !e.target.closest('.btc-masthead-btn')) {
      state.btcOpen = false; render();
    }
  });
  ...
  async function fetchLiveFeed() {
    ...
    try {
      ...
      state.feedLoading = false; render();
    }
  }
  ...
  const ICONS = {
    ...
    btc: '<svg width="11" height="11" viewBox="0 0 24 24" fill="currentColor"><path d="M23.638 14.904c-1.602 6.43-8.113 10.34-14.542 8.736C2.67 22.05-1.244 15.525.362 9.105 1.962 2.67 8.475-1.243 14.9.358c6.43 1.605 10.342 8.115 8.738 14.546zM15.73 7.364c-.93-.36-1.944-.594-2.954-.713l.53-2.11-1.295-.324-.516 2.054a42.4 42.4 0 0 0-1.036-.248l.52-2.07-1.294-.323-.53 2.108c-.285-.065-.566-.13-.838-.198l.002-.007-1.786-.446-.344 1.384s.956.218.936.232c.522.13.617.476.601.75l-.602 2.41c.036.01.082.023.133.044l-.135-.034-.844 3.376c-.064.158-.226.396-.59.306.013.018-.936-.234-.936-.234l-.64 1.483 1.686.42c.314.08.62.162.923.24l-.536 2.15 1.293.322.53-2.112c.347.094.683.181 1.012.263l-.528 2.1 1.294.324.536-2.144c2.21.418 3.873.25 4.573-1.75.563-1.607-.028-2.534-1.19-3.138.847-.196 1.485-.754 1.655-1.907zm-2.963 4.153c-.4 1.607-3.107.738-3.984.52l.71-2.847c.878.22 3.694.654 3.274 2.327zm.401-4.176c-.365 1.463-2.62.72-3.352.538l.644-2.579c.732.183 3.088.523 2.708 2.04z"/></svg>',
    road: '<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3 17l3-10 6 3 6-3 3 10"/><line x1="12" y1="10" x2="12" y2="17"/></svg>',
    ticket: '<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M2 9a3 3 0 0 1 0 6v2a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2v-2a3 3 0 0 1 0-6V7a2 2 0 0 0-2-2H4a2 2 0 0 0-2 2Z"/><path d="M13 5v2"/><path d="M13 17v2"/><path d="M13 11v2"/></svg>',
  };
    ...
    return `<div class="masthead">
      ...
      <div class="masthead-right">
        <div style="position:relative;">
          <button class="toggle-btn btc-masthead-btn ${state.btcOpen?'btc-open':''}" onclick="MetalTape.toggleBtc(event)" title="Support with Bitcoin">
            ${ICONS.btc} <strong>DONATE BTC</strong>
          </button>
          ${state.btcOpen ? `<div class="btc-panel-masthead" onclick="event.stopPropagation()">
            <div class="btc-panel-title">⛧ SUPPORT THE TAPE ⛧</div>
            <div class="btc-panel-sub">BITCOIN DONATION ADDRESS</div>
            <div class="btc-panel-address" onclick="MetalTape.copyBtc(event)">${BTC_ADDRESS}</div>
            <div class="btc-panel-hint">${state.btcCopied ? '✓ COPIED TO CLIPBOARD' : '⛧ TAP ADDRESS TO COPY ⛧'}</div>
            <button class="btc-panel-close" onclick="MetalTape.toggleBtc(event)">CLOSE</button>
          </div>` : ''}
        </div>
  ...
  function tplFeed() {
    if (state.feedLoading) {
      return `<div class="empty-state">
        ...
        <div class="empty-state-sub">Pulling from 17 sources...</div>
      </div>`;
    }
  ...
  function tplBottom() {
    return `<div class="bottom-bar">
      <div>⛧ 17 SOURCES · ${state.news.length} STORIES · ${state.watchlist.length} WATCHED ⛧</div>
